...
# Valor del threshold controlado desde la pagina web (voltios, 0.0 a 3.3)
threshold_actual = 1.65
...
@app.route("/control_pwm", methods=["POST"])
def control_pwm():
...
def set_threshold():
    ...
    try:
        ...
        # Limitar entre 0 y 3.3 voltios (VREF del ADC del ESP32-S3)
        threshold_actual = max(0.0, min(3.3, valor))
